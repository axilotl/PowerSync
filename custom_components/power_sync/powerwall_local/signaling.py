...
class TeslaSignalingClient:
    ...
    async def send_signed_command(self, signed_bytes: bytes) -> bool:
        """Send a signed RoutableMessage via the WebSocket as SIGNED_COMMAND.

        Opens a one-shot WebSocket connection if not already connected,
        wraps the signed bytes in a CommandMessage with type 1047, sends
        it, waits briefly for a response, then returns.

        This bypasses the REST device_command path entirely — the signed
        protobuf goes directly through the Hermes channel to the gateway.
        """
        # Build CommandMessage protobuf:
        # field 1 (txid): random bytes
        # field 5 (commandType): 1047 (SIGNED_COMMAND)
        # field 10 (payload): signed RoutableMessage bytes
        # field 3 (topic): "energy_device.{din}.cmds"
        txid = str(uuid.uuid4()).encode()
        topic = f"energy_device.{self._din}.cmds".encode()

        inner = b""
        # field 1 (txid): wire type 2
        inner += _write_varint((1 << 3) | 2) + _write_varint(len(txid)) + txid
        # field 3 (topic): wire type 2
        inner += _write_varint((3 << 3) | 2) + _write_varint(len(topic)) + topic
        # field 5 (commandType): wire type 0
        inner += _write_varint((5 << 3) | 0) + _write_varint(CommandType.SIGNED_COMMAND)
        # field 10 (payload): wire type 2
        inner += (
            _write_varint((10 << 3) | 2)
            + _write_varint(len(signed_bytes))
            + signed_bytes
        )
        # Wrap in HermesMessage field 1
        msg = _write_varint((1 << 3) | 2) + _write_varint(len(inner)) + inner

        # Reset auth_denied so we can try connecting
        self._auth_denied = False
        ...
        if self.is_connected and self._ws is not None:
            try:
                ...
                _LOGGER.info(
                    "signaling: sent SIGNED_COMMAND (%d bytes) via existing connection",
                    len(msg),
                )
                return True
            except Exception as err:
                _LOGGER.warning("signaling: send failed on existing ws: %s", err)

        # Open a one-shot connection
        _LOGGER.info("signaling: opening one-shot WebSocket for SIGNED_COMMAND")
        jwt = await self._get_hermes_jwt()
        if not jwt:
            _LOGGER.warning("signaling: no JWT available for one-shot send")
            return False
        ...
        try:
            async with aiohttp.ClientSession() as sess:
                async with sess.ws_connect(
                    ...
                    _LOGGER.info("signaling: one-shot WebSocket connected")
                    # Read initial message (STREAMING_CONFIG) and ACK it
                    try:
                        init_msg = await asyncio.wait_for(ws.receive(), timeout=5)
                        if init_msg.type == aiohttp.WSMsgType.BINARY:
                            fields = _parse_hermes_message(init_msg.data)
                            if fields:
                                init_txid = fields.get(1)
                                if isinstance(init_txid, bytes):
                                    ack = _build_client_ack(init_txid)
                                    await ws.send_bytes(ack)
                    except asyncio.TimeoutError:
                        pass

                    # Send the signed command
                    await ws.send_bytes(msg)
                    _LOGGER.info(
                        "signaling: sent SIGNED_COMMAND (%d bytes) via one-shot",
                        len(msg),
                    )

                    # Wait for response
                    try:
                        resp_msg = await asyncio.wait_for(ws.receive(), timeout=10)
                        if resp_msg.type == aiohttp.WSMsgType.BINARY:
                            resp_fields = _parse_hermes_message(resp_msg.data)
                            if resp_fields:
                                cmd_type = resp_fields.get(5)
                                status_code = resp_fields.get(7)
                                _LOGGER.info(
                                    "signaling: response cmd_type=%s status=%s",
                                    cmd_type, status_code,
                                )
                                # ACK the response
                                resp_txid = resp_fields.get(1)
                                if isinstance(resp_txid, bytes):
                                    await ws.send_bytes(_build_client_ack(resp_txid))
                    except asyncio.TimeoutError:
                        _LOGGER.info("signaling: no response within 10s")

                    return True

        except Exception as err:
            _LOGGER.error("signaling: one-shot WebSocket failed: %s", err)
            return False
